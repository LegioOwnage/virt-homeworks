...
---
Ответ:
Видимо с российских ip нет доступа к репозиторию hashicorp, поэтому ответ прилагаю просто командами, которые бы вводил.
Посоветуйте vpn для ubuntu :)
```bash
Смотрим доступные версии:
apt policy terraform
Устанавливаем дополнительную версию из репозитория:
sudo apt install terraform=0.12.31
```
